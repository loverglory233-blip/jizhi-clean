...
settings.ip = "0.0.0.0";
settings.port = 9001;
settings.minify = false;       // ⚠️ 保持 false：项目使用 12 个社区插件(ep_tables4/ep_font_color等)，minify 会导致部分插件静默失效
settings.maxAge = 21600000;    // ⚡ 静态外壳资源 6 小时强缓存 (21600000ms)，二次访问 0 带宽消耗
