...
import { AgentProfiles, TASK_GENRE_CONFIGS, getAgentDisplayName, APP_VERSION } from "./constants.js?v=20260908_v2880";
import { callCozeAgentAPI } from "./agents.js?v=20260908_v2880";
import { downloadFileBlob, getCaretCharacterOffsetWithin, setCaretPositionWithin, escapeHtml, sanitizeUrl, isTaskExpired, formatDurationHuman, formatChatDisplayTime, filterAndDeduplicateChatLogs, enforceEtherpadReadonly, liftEtherpadReadonly, ensureEtherpadUserSync, getUserAllKeys, isSameUser, isUserInMap, getUserFromMap, isMemberDone, isScopeMatch, showResolutionBlock, isSameId } from "./utils.js?v=20260908_v2880";
 ...
 * 🛡️ 成员贡献比取值辅助函数
 */
export const getMemberContribVal = (contribs, m) => {
 ...
 */
export function getEtherpadAuthorStats(frameId = 'stage2-etherpad-frame', membersListParam = null, currUserNameParam = null, stateParam = null) {
  try {
    const app = window.app;
    ...
    if (!isOnline) {
      for (const k of candidateKeys) {
        ...
        if (p) {
          ...
          if (pTime > 0 && ((serverNow - pTime <= 45000) || (pTime - serverNow <= 45000))) {
            isOnline = true;
            break;
          }
        }
    ...
    if (isTaskDeadlineExpired) {
      if (!expiredBanner) {
        ...
        b.innerHTML = `
          <div style="display:flex; align-items:center; gap:8px; min-width:0; flex:1; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">
            <span style="font-size:15px; flex-shrink:0;">🔒</span>
            <span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><b>任务已截止锁定：</b> 本任务已于 <b>${currentTask?.deadline || '截止时间'}</b> 截止，阶段三【${taskGenreKey === 'instructional' ? '答辩评审会' : '答辩擂台'}】已自动转为<b>【只读查阅模式】</b>。如需修改请联系教师延长时间。</span>
          </div>
          <span style="font-size:11.5px; color:#ffffff; background:#dc2626; padding:2px 8px; border-radius:4px; font-weight:800; flex-shrink:0; letter-spacing:0.5px;">已截止</span>
        `;
...
export function renderReferencePapersModal() {}
...
export function initEtherpadIntegration() {}
export function enforceEtherpadWorkspaceGuard() {}
export function renderPresenceCursors() {}
