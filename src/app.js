...
import {
  ...
  AgentProfiles
} from "./constants.js?v=20260830_v860";
import { downloadFileBlob, escapeHtml, getCaretCharacterOffsetWithin, isTaskExpired, showGlobalBannerNotice, formatStandardDateDash } from "./utils.js?v=20260830_v860";
import { callCozeAgentAPI } from "./agents.js?v=20260830_v860";
import { AuthManager } from "./auth.js?v=20260830_v860";
import { CloudSyncEngine } from "./sync.js?v=20260830_v860";
import { renderLoginView } from "./login.js?v=20260830_v860";
import { renderTeacherPortal } from "./teacher.js?v=20260830_v860";
import { renderStudentTaskPortal } from "./student-portal.js?v=20260830_v860";
import {
  buildWordEditorHtml,
  ...
  renderPresencePills,
  renderRemoteCursors
} from "./editor.js?v=20260830_v860";
      ...
      if (stage === 'stage1') {
        ...
        if (submittedCount > 0 && submittedCount < totalMembersCount) {
          ...
          if (now - lastProposalTime > 210000) {
            ...
            if (count < 1 && (!this.lastPartialProposalNudgeTime || now - this.lastPartialProposalNudgeTime > 300000)) {
        ...
        if (submittedCount >= totalMembersCount && votesCastCount < totalMembersCount) {
          ...
          if (shouldVoteNudge) {
            ...
            if (count < 1 && (!this.lastVoteNudgeTime || now - this.lastVoteNudgeTime > 300000)) {
...
请通读以上小组成员的真实研讨发言，作为学术编辑部责任编辑发表一段 100~130 字的【一致性研讨小结与交棒】：简明肯定大家对齐的修改思路，并隆重引出审稿专家通读草稿下发 3 项修正清单！（纯自然语言，严禁输出代码块）`;
        ...
        // 4) 二审修正清单下发后的静默跟进提示 (独家全包给审稿编辑，最多2次)
...
② 针对该融合题目给出 2 个具体细化方向，引导大家继续在讨论区交流深化，暂不急于填表。纯自然语言，90~110字。`;
        ...
        // 2. 若处于【方案细化】状态：40s物理静默期满后，100%由大模型通读真实发言进行学术研判，提炼亮点并引导分工与时间规划
...
（纯自然语言输出，100~130字，严禁输出代码块或技术标记）`;
          ...
          // ⏱️ 物理防抖：只要组员在交流方案，秒表不断打断重置；只有全组完全停下来满 40 秒无新发言，大模型才出面通读总结！
          this._s1RefineTimer = setTimeout(triggerTaskGuidance, 40000);
        }
        // 3. 若处于【分工与时间商议】状态：40s物理静默期满后，100%由大模型通读分工记录进行确认与提醒生成草案
...
（要求：干练有力、亲切鼓舞；纯自然语言输出，80~100字，严禁输出代码块或技术标记）`;
          ...
          // ⏱️ 物理防抖：只要组员在商量分工，秒表不断打断重置；只有全组完全停下来满 40 秒无新发言，大模型才出面通读确认并提醒生成草案！
