...
【本次即时指令】:
1. 答辩陈述（80~120字）：当前为【研究方案/论文设计阶段，尚无实测数据】！严禁捏造或假定已有数据、具体统计系数（如α值）；以组员上述讨论（如引用文献）为绝对核心，适度补充 1~2 条契合方案阶段的规范动作（如补充引用量表经典文献、在正文明确量表来源与维度界定、细化实施步骤），切实解决反方质疑。
2. 主席发言（120~160字）：定案本题；${remainingOppCount > 0 ? `顺推【${nextLabel}】，必须明确引述反方针对该题的核心质疑原文（“${nextInqFullContent.slice(0, 80)}...”），并给出针对性破局思路。` : `宣布全部质询辩护完毕，提醒全员在右上方点击【✍️ 确认答辩完成】进入修改终稿。`}
