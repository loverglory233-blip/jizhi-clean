...
import {
  ...
  AgentProfiles
} from "./constants.js?v=20260907_v2866";
import { downloadFileBlob, escapeHtml, getCaretCharacterOffsetWithin, isTaskExpired, showGlobalBannerNotice, showTaskExtendedUnlockModal, showTaskDeadlineExpiredModal, liftEtherpadReadonly, enforceEtherpadReadonly, formatStandardDateDash, getUserAllKeys, isSameUser, isUserInMap, getUserFromMap, isMemberDone, isScopeMatch, showResolutionBlock, safeJsonParse, parseMsgTime, filterAndDeduplicateChatLogs, isSameId, normalizeId, flashHighlightElement } from "./utils.js?v=20260907_v2866";
import { callCozeAgentAPI } from "./agents.js?v=20260907_v2866";
import { AuthManager } from "./auth.js?v=20260907_v2866";
import { CloudSyncEngine } from "./sync.js?v=20260907_v2866";
import { renderLoginView } from "./login.js?v=20260907_v2866";
import { renderTeacherPortal } from "./teacher.js?v=20260907_v2866";
import { renderStudentTaskPortal } from "./student-portal.js?v=20260907_v2866";
import {
  renderEditor,
  renderChat,
  renderChatActionBar,
...
} from "./editor.js?v=20260907_v2866";
  ...
  async triggerStage2FirstReview(btnElement = null) {
    ...
    try {
      ...
      try {
        ...
        console.warn('triggerStage2FirstReview API call error:', e);
      }
  ...
  async handleOneClickGenerateContract() {
    if (this.isAnyExtracting()) {
      ...
      if (isStale) {
        this.setActiveAgentAnalyzing(null);
        this._isGeneratingContract = false;
        if (this._extractingTimestamps) this._extractingTimestamps['contract'] = null;
      } else {
        if (typeof showGlobalBannerNotice === 'function') {
          showGlobalBannerNotice('⏳ 正在提炼中', '智能体当前正在分析提炼中，请稍候完成后再操作！', 'info', 3000);
        }
...
输出格式必须为合法 JSON（严禁代码块以外的多余文字）：
{
  ...
  "guideText": "太棒了！公约草案已全部生成就绪！请全组成员核对左侧公约并在下方点击【✍️ 签署确认${contractTitle}】！"
    ...
    this.state.chatLogs.stage1 = (this.state.chatLogs.stage1 || []).filter(m => {
      if (!m) return false;
      if (m.isThinking || String(m.id || '').startsWith('thinking_full_contract_') || String(m.id || '').startsWith('err_full_contract_') || String(m.id || '').startsWith('err_contract_')) return false;
      const txt = m.text || '';
    ...
    if (this.state.stage1) {
      this.state.stage1._contractGenerateFailed = false;
      this.state.stage1._fullContractFailed = false;
    }
...
（纯自然语言输出，120~160字，严禁输出代码块）`;
      ...
      this.setActiveAgentAnalyzing({ icon: '📝', title: `【${reviewingName}】正在下发《${isInst ? '磨课修正清单' : '二审修正清单'}》...`, detail: '正在通读全篇草稿并结合自查瓶颈，生成包含【诊断问题+改进建议】的双结构清单...' });
      await new Promise(r => setTimeout(r, 1200));
...
态度客观严谨、温和建设，纯自然语言输出，200~260字。`;
        ...
        try {
          ...
          propText = (pResult && typeof pResult === 'string') ? pResult.trim() : '';
          oppText = (oResult && typeof oResult === 'string') ? oResult.trim() : '';
        } catch (e) {
        ...
        if (!propText || !oppText) {
          ...
          const errPipelineMsg = {
            ...
            sender: 'neutral',
            senderName: chairName,
            text: `⚖️ 【${chairShort}·提醒】：📡 正反方评审大模型通读全篇生成未完成或网络异常。<br><button class="btn-retry-ai" onclick="window.app.runStage3CommitteePipeline(this)" style="margin-top:6px; background:#2563eb; color:#fff; border:none; padding:5px 14px; border-radius:12px; font-size:12px; cursor:pointer; font-weight:700;">🔄 重新调用大模型审阅全篇</button>`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            _timeMs: Date.now()
          };
        ...
        // 🛡️ 清理历史残留的网络提醒错误气泡
        this.state.chatLogs.stage3 = (this.state.chatLogs.stage3 || []).filter(m => !m || !(m.text || '').includes('网络提醒') && !(m.text || '').includes('大模型通读全篇生成未完成'));
...
③ 引导全组在讨论区充分商讨，商定差不多后点击聊天框上方【💡 意见 1 讨论差不多了？帮我总结并填入】按钮！纯自然语言输出，130~150字。`;
        ...
        if (!chairText) {
          const errChairMsg = {
            ...
            sender: 'neutral',
            senderName: chairName,
            text: `🟡 【${chairShort}·提醒】：📡 答辩思路引导大模型生成未完成。<br><button class="btn-retry-ai" onclick="window.app.runStage3CommitteePipeline(this)" style="margin-top:6px; background:#d97706; color:#fff; border:none; padding:5px 14px; border-radius:12px; font-size:12px; cursor:pointer; font-weight:700;">🔄 点击重新生成答辩思路引导</button>`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            _timeMs: Date.now()
          };
        ...
        // 🛡️ 清理历史残留的网络提醒错误气泡
        this.state.chatLogs.stage3 = (this.state.chatLogs.stage3 || []).filter(m => !m || !(m.text || '').includes('网络提醒') && !(m.text || '').includes('生成未完成'));
...
④ 纯自然语言，80~110字，严禁指责，【绝对严禁出现“分工”字眼】，强调共同思考与协同衔接，严禁输出代码块，严禁添加按钮。`;
  ...
  showStageMilestoneModal({ icon = '🎉', title, subtitle, targetName, onProceed }) {
    ...
    let remainingSec = 5;
    modal.innerHTML = `
      <div style="background:#ffffff; width:92%; max-width:500px; border-radius:16px; box-shadow:0 24px 48px rgba(15,23,42,0.3); border:1px solid #cbd5e1; overflow:hidden; display:flex; flex-direction:column; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; text-align:center; overscroll-behavior:contain; position:relative;">
        <button id="btn-close-milestone-x" style="position:absolute; top:12px; right:12px; background:rgba(255,255,255,0.2); border:1px solid rgba(255,255,255,0.35); color:#ffffff; width:28px; height:28px; border-radius:8px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:14px; z-index:10; transition:all 0.15s ease;" title="关闭">✕</button>
    ...
    modal.querySelector('#btn-close-milestone-x')?.addEventListener('click', proceed);
