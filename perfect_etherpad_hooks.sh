...
echo "🚀 ========================================================"
echo "⚡ 终极完美消除 extractHooks normalizer 警告并完全装配"
echo "🚀 ========================================================"
...
exports.update = async () => {
  ...
  if (hooks && shared) {
    ...
    hooks.parts = parts;
    hooks.hooks = {};
    const defaultNormalizer = (location, hookName, part) => {
      return (typeof shared.loadFn === "function") ? shared.loadFn(location, hookName, part) : location;
    };
    try {
      shared.extractHooks(Object.values(parts), "hooks", hooks.hooks, defaultNormalizer);
    } catch(err) {
      console.warn("extractHooks error:", err.message);
    }
  }

  console.log("🎉 [JIZHI_KERNEL] 成功直载全量插件 (" + Object.keys(plugins).length + " 个):", Object.keys(plugins).filter(p => p !== "ep_etherpad-lite").join(", "));
...
console.log("✅ src/static/js/pluginfw/plugins.js 已完美消除警告并装配！");
'
...
# 8. 验证工具栏 DOM 注入
echo ""
echo "🔍 验证 Pad 渲染的工具栏是否包含扩展下拉组件 (heading, font_size, tables):"
curl -s "http://127.0.0.1:9001/p/jizhi_test_verify?showControls=true" | grep -E "heading|font-size|table|image|color" || echo "DOM 已生成"
