...
**版本号**: v2221 (`20260904_v2221`)
**更新时间**: 2026-09-04
**更新类型**: 阶段一选题速评重试按键按需展示优化（仅在网络延迟或AI失败时出现）

---

## 🔄 阶段一选题速评重试按键按需展示优化 (20260904_v2221)

### 1. 提案卡片“重新请求速评”按键按需展示
- **优化前**: 组员提交选题提案后，卡片上默认立即常驻显示【🔄 重新请求速评】按钮，造成界面冗余且容易误触。
- **优化后**: 
  1. 初始提交或修改选题时，默认不显示重试按钮。
  2. 只有当大模型评估因网络超时、连接异常或返回空内容导致速评未成功生成时，卡片上才会即时唤起重试按钮（并高亮提示）。
  3. 用户点击重试并成功获取学术速评后，重试按钮自动消失，界面恢复整洁清爽。
